...
### Enable UC5 Tier B LLM discovery (OpenRouter)

When Tier A catalog search and Tier C ML discovery return no SAFE substitutes, Tier B asks a chat model for candidate barcodes, resolves them against the local catalog, and logs an audit row. The dietary rule engine still decides SAFE/WARNING/UNSAFE.

OpenRouter uses the existing Spring AI OpenAI-compatible client:

```powershell
$env:OPENAI_BASE_URL = "https://openrouter.ai/api/v1"
$env:OPENAI_API_KEY = "sk-or-v1-your-openrouter-key"
$env:OPENAI_MODEL = "openai/gpt-4o-mini"
$env:CANMAKAN_RECOMMENDATION_LLM_ENABLED = "true"
.\mvnw.cmd spring-boot:run
```

| Variable | Default | Tier B purpose |
|----------|---------|----------------|
| `OPENAI_BASE_URL` | `https://api.openai.com/v1` | Set to `https://openrouter.ai/api/v1` for OpenRouter |
| `OPENAI_API_KEY` | `local-dev-placeholder` | OpenRouter or OpenAI API key |
| `OPENAI_MODEL` | `gpt-4o-mini` | Model slug, e.g. `openai/gpt-4o-mini` |
| `CANMAKAN_RECOMMENDATION_LLM_ENABLED` | `false` | Set `true` to activate Tier B |

Tier C ML (no API key) is controlled separately:

| Variable | Default | Tier C purpose |
|----------|---------|----------------|
| `CANMAKAN_RECOMMENDATION_ML_ENABLED` | `true` | Set `false` for Tier A-only behavior |
...
Fallback order: Tier A → Tier C (also expands if fewer than 5 SAFE items, using labels and sibling categories) → Tier B → empty response. LLM failures are logged and skipped (same pattern as Tier-3 assess).

Additive catalog tag backfill lives in `01c_recommendation_substitute_tags.sql` (generated by `server/machine-learning/scripts/audit_substitute_tags.py`).
