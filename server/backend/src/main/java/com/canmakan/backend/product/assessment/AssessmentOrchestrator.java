...
public class AssessmentOrchestrator {
    ...
    public AssessmentResponse assess(Long userId, AssessmentRequest request) {
        ...
        if (llmResult == null) {
            aiExecutionLogService.recordRulesOnly(scan.getId(), rulesLatencyMs);
        } else {
    ...
    private SafetyVerdict aggregateModelEvidence(
        ...
        List<Finding> combined = new ArrayList<>(deterministicVerdict.findings());
        llmResult.proposedFindings().stream()
                .filter(finding -> finding.restrictionCode() == null
                        || applicableCodes.contains(finding.restrictionCode()))
