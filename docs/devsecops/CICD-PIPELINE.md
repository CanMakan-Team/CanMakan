...
Security jobs in CI: **Gitleaks** (secrets), **Semgrep** (SAST, including GitHub Actions YAML), **Trivy fs** (SCA on the repo tree), **Trivy image** (SCA on backend and ML runtime images). **SonarCloud** is the maintainability / coverage quality gate on each stack build. **Gitar** (`gitar-bot`) reviews pull requests on GitHub. Post-deployment security and load testing (OWASP ZAP and Grafana k6) run against the live Staging environment on automated schedules.
...
| ------------------ | ---------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Secret scanning    | Gitleaks **8.21.2**          | `ci.yml` job `gitleaks`                                             | `.gitleaks.toml` (`[allowlist]`, `useDefault = true`); `.gitleaksignore`; `fetch-depth: 0`; SARIF to Code Scanning (`gitleaks`) plus artefact `gitleaks-sarif`                                                                 | Built for git history and one allowlist. Trivy `fs` can also flag secrets; we turn that off so Trivy red means CVEs, not the test JWT Gitleaks allowlists.                                                                                                                                                   |
| SAST               | Semgrep **1.173.0**          | `ci.yml` job `sast-scan`                                            | Docker `semgrep/semgrep:1.173.0 semgrep --config p/default --sarif-output`; `.semgrepignore` (build outputs, test JWT properties, demo-user SQL, and specific tests that only hold the sample BCrypt hash, the allowlisted test JWT, or `Class.forName` on a hardcoded class list); SARIF to Code Scanning (`semgrep`) plus artefact `semgrep-sarif` | Fast security-pattern scan on every PR, including GitHub Actions YAML (for example SHA pins). There is no separate Trivy config job: Trivy 0.70 on this action pin does not scan workflow files. CodeQL would be a second, slower SAST. SonarCloud is the quality gate, not this SAST job. |
...
| SCA (upgrade PRs)  | Dependabot                   | GitHub                                                              | `.github/dependabot.yml`: weekly Monday, npm / Maven / Gradle / Actions; `cooldown.default-days: 7` on each ecosystem                                                                                      | Answers “move us off old versions **before** they become a gate failure.” Cooldown waits a week after a version is published so a yanked or malicious release is less likely to land in a PR.                                                                                                              |
| Quality gate       | SonarCloud                   | Inside `build-backend`, `build-web`, `build-mobile`, `build-machine-learning` | Org `canmakan-team`. Keys `canmakan-backend`, `canmakan-web`, `canmakan-mobile`, `canmakan-ml`. `sonar.qualitygate.wait=true`                                                                      | Semgrep does not rate complexity, duplication, or coverage. That gate belongs here. The report is the SonarCloud dashboard, not a CI artefact.                                                                                                                                                               |
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `[ci.yml](../../.github/workflows/ci.yml)`                             | PR / push to `develop` and `main`, `workflow_dispatch`                             | Gitleaks, Semgrep, Trivy fs, path-filtered backend / web / mobile / ML builds + SonarCloud, backend + ML images + Trivy image, **Build Test**, `backend-jar` / `ml-image` upload, GHCR push on `push` or `workflow_dispatch` to `develop`/`main` |
| `[e2e.yml](../../.github/workflows/e2e.yml)`                           | PR to `develop`/`main`, push to `develop`, `workflow_dispatch`                     | Playwright when `client/web/**` changes                                                                                                                                                                                  |
| `[deploy.yml](../../.github/workflows/deploy.yml)`                     | `workflow_run` after successful CI **push** or **dispatch** on `main`/`develop` with `backend-jar` or `ml-image` | EC2 container deploy of backend and/or ranker. Dynamically targets Staging or Production.                                                                                                                                |
...
| SARIF upload         | Gitleaks, Semgrep, and Trivy jobs upload SARIF to Code Scanning and (except empty image placeholders) as Actions artefacts. On a **public** repo this works without GitHub Advanced Security. Trivy filesystem/image failure is still the **table** scan (`exit-code: 1`).                                                                                                 |
| Action pins          | All `uses:` in `.github/workflows/` are full 40-character commit SHAs (version in a comment). Stops Semgrep `github-actions-mutable-action-tag` and tag-move supply-chain risk. Dependabot still bumps GitHub Actions; merge the SHA it proposes.                                                                                                                              |
...
Concurrency: `ci-${{ github.ref }}`. Workflow permissions: `contents: read`, `pull-requests: read`. Gitleaks, Semgrep, and Trivy jobs also set `security-events: write` so SARIF can upload.
...
| `code-scanning-skipped-image-sarif` | PRs that skip backend and/or ML image jobs | Uploads empty SARIF for `trivy-image` / `trivy-ml-image` so GitHub can still diff Code Scanning against `develop`/`main` when those jobs are path-filtered. Not a security gate. |
| `gitleaks`               | always                       | `fetch-depth: 0`, Gitleaks **8.21.2**, SARIF to Code Scanning plus artefact `gitleaks-sarif`                                                                                                                                                             |
| `sast-scan`              | always                       | Docker `semgrep/semgrep:1.173.0 --config p/default` (app code and `.github/workflows`), SARIF to Code Scanning plus artefact `semgrep-sarif`                                                                                                           |
| `sca-scan`               | always                       | Trivy `fs`, `scanners: vuln` only, CRITICAL/HIGH, table + SARIF to Code Scanning plus artefact `trivy-fs-sarif`; CycloneDX SBOM artefact `trivy-sbom`                                                                                                    |
| `build-backend`          | backend paths, or CI dispatch with `force_backend` | JDK 21, MySQL **8.0** service, `mvn -B clean verify` (not RDS) + JaCoCo; SonarCloud; Docker image from the verified JAR; Trivy **image**; GHCR push on `push` or `workflow_dispatch` to `develop`/`main`; uploads `backend-jar` and artefact `trivy-image-sarif`                                                |
| `build-machine-learning` | ML paths, product seed SQL, or CI dispatch with `force_ml` | Python **3.12**, pytest coverage gate + XML for Sonar, SonarCloud `canmakan-ml`, train `tfidf_ranker.joblib`, Docker image, Trivy **image**, GHCR push on `push` or `workflow_dispatch` to `develop`/`main`; uploads `ml-image` marker and artefact `trivy-ml-image-sarif`                                         |
...
| Job                | When                       | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------ | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `resolve-artifact` | Successful CI **push** or **dispatch** | `gh api` lists artefacts for that CI `run-id`; sets `has_jar` and `has_ml`                                                                                                                                                                                                                                                                                                                                                                                          |
