...
| IaC / config       | Trivy config                 | `ci.yml` job `config-scan`                                          | `scan-type: config`, `scanners: misconfig`, `scan-ref: .github/workflows` with `TRIVY_MISCONFIG_SCANNERS: github-actions` and `TRIVY_FILE_PATTERNS` for `*.yml` / `*.yaml`. Those env vars are set on the job step in `ci.yml`, not as GitHub secrets. CRITICAL/HIGH. | Same Trivy family, different scan type: misconfigured GitHub Actions workflow YAML. Dependabot and other `.github` files are out of scope.                                                                                                                                                                   |
...
| `config-scan`            | always                       | Trivy `config` / `misconfig` on `.github/workflows/*.yml` and `*.yaml` (`github-actions` scanner), CRITICAL/HIGH, SARIF to Code Scanning plus artefact `trivy-config-sarif`                                                                                                                                 |
