...
| Quality gate       | SonarCloud                   | Inside `build-backend`, `build-web`, `build-mobile`, `build-machine-learning` | Org `canmakan-team`. Keys `canmakan-backend`, `canmakan-web`, `canmakan-mobile`, `canmakan-ml`. `sonar.qualitygate.wait=true`                                                                      | Semgrep does not rate complexity, duplication, or coverage. That gate belongs here.                                                                                                                                                                                                                          |
...
| `build-backend`          | backend paths, or CI dispatch with `force_backend` | JDK 21, MySQL **8.0** service, `mvn -B clean verify` (not RDS) + JaCoCo; SonarCloud; Docker image from the verified JAR; Trivy **image**; GHCR push on `push` or `workflow_dispatch` to `develop`/`main`; uploads `backend-jar`                                                |
| `build-machine-learning` | ML paths, product seed SQL, or CI dispatch with `force_ml` | Python **3.12**, pytest coverage gate + XML for Sonar, SonarCloud `canmakan-ml`, train `tfidf_ranker.joblib`, Docker image, Trivy **image**, GHCR push on `push` or `workflow_dispatch` to `develop`/`main`; uploads `ml-image` marker                                         |
