...
| SAST               | Semgrep **1.173.0**          | `ci.yml` job `sast-scan`                                            | Docker `semgrep/semgrep:1.173.0 semgrep --config p/default --sarif-output`; `.semgrepignore` (build outputs, test JWT properties, all `server/backend/src/main/resources/*.sql` seed files); SARIF to Code Scanning (`semgrep`) plus artefact `semgrep-sarif` | Fast security-pattern scan on every PR. CodeQL would be a second, slower SAST. SonarCloud is the quality gate, not this SAST job.                                                                                                                                                                            |
...
| IaC / config       | Trivy config                 | `ci.yml` job `config-scan`                                          | `scan-type: config`, `scanners: misconfig`, `scan-ref: .github/workflows` with `TRIVY_MISCONFIG_SCANNERS: yaml` and `TRIVY_FILE_PATTERNS` for `*.yml` / `*.yaml`. Trivy 0.70 has no `github-actions` scanner (that name is ignored and yields 0 files). Those env vars are set on the job step, not as GitHub secrets. CRITICAL/HIGH. | Same Trivy family, different scan type: workflow YAML misconfig. Semgrep still owns Actions *code* patterns (for example SHA pins). Dependabot YAML is out of scope.                                                                                                                                           |
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `[ci.yml](../../.github/workflows/ci.yml)`                             | PR / push to `develop` and `main`, `workflow_dispatch`                             | Gitleaks, Semgrep, Trivy fs, Trivy config, path-filtered backend / web / mobile / ML builds + SonarCloud, backend + ML images + Trivy image, **Build Test**, `backend-jar` / `ml-image` upload, GHCR push on `push` or `workflow_dispatch` to `develop`/`main` |
...
| `sast-scan`              | always                       | Docker `semgrep/semgrep:1.173.0 --config p/default`, SARIF to Code Scanning plus artefact `semgrep-sarif`                                                                                                                                               |
...
| `config-scan`            | always                       | Trivy `config` / `misconfig` on `.github/workflows/*.yml` and `*.yaml` (`yaml` scanner; Trivy 0.70), CRITICAL/HIGH, SARIF to Code Scanning plus artefact `trivy-config-sarif`                                                                                                                              |
