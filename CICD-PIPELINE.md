...
CanMakan utilises **GitHub Actions** to manage a monorepo comprising of 3 products: Spring Boot (`server/backend`), React/Vite (`client/web`), and Kotlin/Android Jetpack Compose (`client/mobile`).
...
* **Code Ownership:** `.github/CODEOWNERS` enforces mandatory reviews from `@Codemelia` and `@K4i-Z3r` for specific architectural boundaries.
