...
| `build-machine-learning` | ML paths or product seed SQL | Python **3.12**, pytest coverage gate + XML for Sonar, SonarCloud `canmakan-ml`, train `tfidf_ranker.joblib`, Docker image, Trivy **image**, GHCR push on `push` to `develop`/`main`; uploads `ml-image` marker                                         |
