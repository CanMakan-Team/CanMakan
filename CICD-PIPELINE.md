...
ML image: the Dockerfile runs `apt-get upgrade` so Debian packages that already have a **Fixed Version** (for example util-linux) must be patched or the job fails. CRITICAL/HIGH with **no Debian fix yet** (perl-base, gzip, ncurses, OpenSSL QUIC, …) are `ignore-unfixed` on the ML table gate only; they still appear in SARIF `trivy-ml-image`. Do not set `exit-code: 0` on that scan.
